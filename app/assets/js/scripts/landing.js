...
document.getElementById('launch_button').addEventListener('click', function (e) {
    ...
    require('electron').shell.openExternal('fivem://connect/103.27.203.59:30123')
...
const refreshServerStatus = async function (fade = false) {
    ...
    const infoURL = 'http://103.27.203.59:30123/info.json'
    const playersURL = 'http://103.27.203.59:30123/players.json'
    ...
    axios.get(infoURL)
        ...
        .then(function () {
            ...
            // document.getElementById('mojangStatusEssentialContainer').innerHTML = tooltipEssentialHTML
            // document.getElementById('mojangStatusNonEssentialContainer').innerHTML = tooltipNonEssentialHTML
            document.getElementById('sv_status').innerHTML = statusLabel

            document.getElementById('mojang_status_icon').style.color = Mojang.statusToHex(status)

            // axios.get('http://103.27.203.92:30210/players.json')
            axios.get(playersURL)
                .then(function (response) {
                    //console.log(response)
                    pLabel = 'PLAYERS'
                    pVal = response.data.length + '/' + maxPlayers
                })
                .catch(function (error) {
                    loggerLanding.warn('Unable to refresh server status, assuming offline.')
                    loggerLanding.debug(err)
